<!-- partials/_prospace_banniere.php -->
<?php
// =============================================================================
// NOM DU SCRIPT : partials/_prospace_banniere.php
// REVISION     : 3.0 - Interface universelle et sélecteur radio simple
// DESCRIPTION  : Interface fluide. Accepte tous les visuels haute résolution.
//                Boutons radio exclusifs pour déverrouiller le paiement du forfait choisi.
// =============================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_update_url_banniere'])) {
    ...
    if ($id_bann_mod > 0 && !empty($nouvelle_url)) {
        try {
            ...
            // Rafraîchir la liste des bannières actives en mémoire
...
// Prix mensuels indépendants lus depuis jevend_tarifs_pro
$prix_mensuel_supreme = (float)($tarifs_pro['supreme'] ?? 129.00);
$prix_mensuel_premium = (float)($tarifs_pro['premium'] ?? 49.00);

// --- COMPTAGE DES BANNIÈRES ACTIVES PAR TYPE & REGROUPEMENT PAR PÉRIODE ---
$nb_supreme_actifs = 0;
$nb_premium_actifs = 0;
...
if (!empty($mes_bannieres_pro)) {
    foreach ($mes_bannieres_pro as $b) {
        ...
        // Comptage des bannières actives
        ...
        // Groupement par Année-Mois (ex: 2026-07)
            ...
            <div class="pro-grid-forfaits">
                ...
                <div class="card-forfait-pro" id="card-supreme" style="border-color: #7c3aed; transition: all 0.3s ease;">
                    <div>
                        ...
                        <?php if ($nb_supreme_actifs >= 3): ?>
                            <div style="background: #fee2e2; color: #991b1b; padding: 15px; border-radius: 6px; text-align: center; font-weight: bold; font-size: 0.85rem; margin: 20px 0; border: 1px solid #fecaca;">
                                ❌ Limite maximale atteinte<br>
                                <span style="font-size: 0.75rem; font-weight: normal;">Vous possédez déjà 3/3 bannières Suprême actives.</span>
                            </div>
                        <?php else: ?>
                    ...
                    <div style="font-size: 0.75rem; color: #64748b; text-align: right; margin-top: 10px;">
                        Actives : <strong><?= $nb_supreme_actifs ?> / 3</strong>
                    </div>
                </div>
                ...
                <div class="card-forfait-pro" id="card-premium" style="border-color: #2563eb; transition: all 0.3s ease;">
                    <div>
                        ...
                        <p style="font-size: 0.85rem; color: #64748b;">
                            Pavés rotatifs haute visibilité sous l'en-tête (20 slots max).
                        </p>
                        
                        <?php if ($nb_premium_actifs >= 5): ?>
                            <div style="background: #fee2e2; color: #991b1b; padding: 15px; border-radius: 6px; text-align: center; font-weight: bold; font-size: 0.85rem; margin: 20px 0; border: 1px solid #fecaca;">
                                ❌ Limite maximale atteinte<br>
                    ...
                    <div style="font-size: 0.75rem; color: #64748b; text-align: right; margin-top: 10px;">
                        Actives : <strong><?= $nb_premium_actifs ?> / 5</strong>
                    </div>
                </div>
                    ...
                    <div class="accordeon-content">
                        <?php foreach ($liste_bannieres as $bann): ?>
                            <?php 
                                ...
                                // CALCUL DYNAMIQUE EN TEMPS RÉEL : 10 jours avant la date de fin réelle
...
<script>
// PRIX MENSUELS DÉDIÉS DÉFINIS DANS JEVEND_TARIFS_PRO
const PRIX_MOIS_SUPREME = <?= $prix_mensuel_supreme ?>; // 129.00 $
const PRIX_MOIS_PREMIUM = <?= $prix_mensuel_premium ?>; // 49.00 $
...
// -----------------------------------------------------------------------------
// GESTIONNAIRE DE SÉLECTION EXCLUSIVE DU FORFAIT (RADIO BUTTONS)
// -----------------------------------------------------------------------------
