...
// DESCRIPTION  : Traitement AJAX léger pour le tchat en direct et indicateur de frappe
// =============================================================================
session_start();
...
// -----------------------------------------------------------------------------
// ACTION 1 : Signal "Je suis en train d'écrire"
// -----------------------------------------------------------------------------
if ($action === 'signal_frappe') {
    try {
        // Enregistre l'horodatage courant de frappe
        $stmt = $bdd->prepare("
            UPDATE jevend_chat 
            SET a_tape_a = NOW() 
            WHERE id_annonce = ? AND id_expediteur = ? AND id_destinataire = ?
...
// -----------------------------------------------------------------------------
// ACTION 2 : Récupération des nouveaux messages + Vérification si l'autre tape
// -----------------------------------------------------------------------------
if ($action === 'fetch_messages') {
    try {
        ...
        $stmt_msgs = $bdd->prepare("
            ...
            WHERE c.id_annonce = ? AND ((c.id_expediteur = ? AND c.id_destinataire = ?) OR (c.id_expediteur = ? AND c.id_destinataire = ?))
        ...
        // 3. Vérifier si l'autre personne est en train d'écrire (activité < 4 secondes)
        $stmt_frappe = $bdd->prepare("
            SELECT COUNT(*) 
            FROM jevend_chat 
            WHERE id_annonce = ? AND id_expediteur = ? AND a_tape_a >= NOW() - INTERVAL 4 SECOND
        ");
        $stmt_frappe->execute([$id_annonce, $id_interlocuteur]);
        $l_autre_ecrit = ($stmt_frappe->fetchColumn() > 0);

        // Formater les données pour JS
        $html_messages = "";
