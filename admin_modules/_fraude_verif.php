...
// REVISION    : 1.0 - Centre de contrôle des signalements et de la fraude
// =============================================================================
...
// Traitement des actions d'administration (Changement de statut ou Bannissement)
$message_admin = "";
$type_message = "";
...
// Récupération de tous les signalements avec jointures (Expéditeur, Cible, Annonce, Recherche)
try {
...
<div style="background: #ffffff; border: 1px solid #cbd5e1; border-radius: 8px; padding: 25px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #f1f5f9; padding-bottom: 15px;">
        <div>
            <h2 style="margin: 0 0 5px 0; color: #1e3a8a;">🚨 Centre de Contrôle de la Fraude</h2>
            <p style="margin: 0; color: #64748b; font-size: 0.88rem;">Surveillance communautaire, signalements et gestion des comportements hors-périmètre.</p>
        </div>
    ...
    <?php if (empty($signalements)): ?>
        <div style="text-align: center; padding: 40px; background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 6px; color: #64748b;">
                ...
                <tbody>
                    <?php foreach ($signalements as $sig): ?>
                        <tr style="border-bottom: 1px solid #e2e8f0; background: <?= ($sig['statut'] === 'en_attente') ? '#fffbeb' : '#ffffff' ?>;">
                            ...
                            <!-- 3, 4, 5. Contexte et Liens vers Annonce / Recherche / Chat -->
                            <td style="padding: 12px 10px; vertical-align: top; white-space: nowrap;">
                                ...
                                <div style="display: flex; gap: 5px; justify-content: center; flex-wrap: wrap;">
                                    ...
                                    <?php if (!empty($sig['id_cible'])): ?>
                                        <form method="POST" style="margin: 0;" onsubmit="return confirm('Attention : Voulez-vous vraiment BANNIR définitivement le membre #<?= $sig['id_cible'] ?> (<?= htmlspecialchars($sig['nom_cible']) ?>) ?');">
                                            ...
                                            <button type="submit" title="Bannir le membre et clôturer" style="background: #dc2626; color: #fff; border: none; padding: 5px 8px; border-radius: 4px; cursor: pointer; font-weight: bold; font-size: 0.75rem;">
                                                🔨 Bannir
                                            </button>
                                        </form>
                                    <?php endif; ?>
