<?php
// =============================================================================
// NOM DU SCRIPT : mes_conversations_recherche.php
// DESCRIPTION  : Liste des conversations actives dédiées au module "Je Cherche"
// =============================================================================
session_start();
...
// Extraction de toutes les conversations actives concernant uniquement les recherches
try {
    $stmt = $bdd->prepare("
        SELECT c.id_recherche, 
               c.id_expediteur, 
               c.id_destinataire,
               c.date_envoi,
               c.lu,
        ...
        FROM jevend_chat c
        JOIN jevend_recherches r ON c.id_recherche = r.id_recherche
        ...
        WHERE (c.id_expediteur = :id_user OR c.id_destinataire = :id_user)
          AND c.id_recherche IS NOT NULL
...
foreach ($tous_messages as $m) {
    ...
    if (!isset($conversations[$cle])) {
        $conversations[$cle] = [
            ...
            'titre_recherche' => $m['titre_recherche'],
...
if (count($conversations_non_lues) === 1) {
    ...
    header("Location: chat_recherche.php?id_recherche=" . $conv_cible['id_recherche'] . "&avec=" . $conv_cible['autre_id']);
    exit();
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Mes conversations "Je Cherche" — jevend.com</title>
...
<body class="admin-body">
    ...
    <div class="liste-conv-container">
        <h2 style="margin-top: 0; color: #0f172a; font-size: 1.3rem;">🎯 Mes conversations « Je Cherche »</h2>

        <?php if (empty($conversations)): ?>
            <p style="color: #64748b; text-align: center; margin: 30px 0;">Vous n'avez aucun échange en cours pour les recherches d'objets.</p>
        <?php else: ?>
            <?php foreach ($conversations as $c): ?>
                <a href="chat_recherche.php?id_recherche=<?= $c['id_recherche'] ?>&avec=<?= $c['autre_id'] ?>" class="item-conv <?= ($c['non_lu'] > 0) ? 'non-lu' : '' ?>">
                    <div>
                        <strong>🎯 <?= htmlspecialchars($c['titre_recherche']) ?></strong>
